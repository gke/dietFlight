...
#endif
static FAST_RAM float accumulatedMeasurements[XYZ_AXIS_COUNT] = {0};
static FAST_RAM int   accumulatedMeasurementCount = 0;
...
static void gyroInitFilterKalman(gyroSensor_t *gyroSensor, uint16_t gyro_filter_q, uint16_t gyro_filter_r, uint16_t gyro_filter_p)
{
    ...
    if (gyro_filter_q != 0 && gyro_filter_r != 0) {
        ...
        for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            fastKalmanInit(&gyroSensor->fastKalman[axis], gyro_filter_q, gyro_filter_r, gyro_filter_p, gyroDt);
        }
    }
}
...
static FAST_CODE void gyroUpdateSensor(gyroSensor_t *gyroSensor, timeUs_t currentTimeUs)
{
    ...
    gyroSensor->gyroDev.dataReady = false;
        ...
        for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            ...
            if (!gyroSensor->overflowDetected) {
                accumulatedMeasurements[axis] += gyroADCf;
...
{
    static float _average[XYZ_AXIS_COUNT] = {0.0f};

    if (accumulatedMeasurementCount > 0) {
        // If we have gyro data accumulated, calculate average rate that will yield the same rotation
        for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            _average[axis] = accumulatedMeasurements[axis] / accumulatedMeasurementCount;
            accumulationAverage[axis] = _average[axis];
...
static void gyroInitSensorFilters(gyroSensor_t *gyroSensor);
#define DEBUG_GYRO_CALIBRATION 3
// gke #define GYRO_OVERFLOW_TRIGGER_THRESHOLD 31980  // 97.5% full scale (1950dps for 2000dps gyro)
// gke #define GYRO_OVERFLOW_RESET_THRESHOLD 30340    // 92.5% full scale (1850dps for 2000dps gyro)
PG_REGISTER_WITH_RESET_TEMPLATE(gyroConfig_t, gyroConfig, PG_GYRO_CONFIG, 1)
;
PG_RESET_TEMPLATE(gyroConfig_t, gyroConfig,
		.gyro_align = ALIGN_DEFAULT,
		...
		.gyro_lpf = GYRO_LPF_NONE,
		.gyro_soft_lpf_type = FILTER_PT1, // should be n=2? gke
...
static FAST_CODE void gyroUpdateSensor(gyroSensor_t *gyroSensor,
	...
	if (isGyroSensorCalibrationComplete(gyroSensor)) {
		...
		for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++)
			gyroSensor->gyroDev.gyroADC[axis] = gyroNewtonianLimiter(gyroSensor,
					axis) - gyroSensor->gyroDev.gyroZero[axis];
